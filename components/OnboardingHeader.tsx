...
export default function OnboardingHeader({ currentStep }: Props) {
    return (
        <View className="px-6 pt-6 pb-4 bg-white border-b border-gray-100">
            ...
            <View className="flex-row items-center justify-between mb-4">
                ...
                <TouchableOpacity onPress={() => router.replace('/(mentor-tabs)')}>
                ...
                {STEPS.map((label, i) => {
                    ...
                    return (
                        <View key={i} className="items-center flex-1">
                            <View
                                className={`w-7 h-7 rounded-full items-center justify-center mb-1 ${isDone
                                        ? 'bg-[#6B46C1]'
                                        : isActive
                                            ? 'bg-[#6B46C1]'
                                            : 'bg-gray-100'
                            ...
                            <Text
                                className={`text-xs text-center ${isActive ? 'text-[#6B46C1] font-semibold' : 'text-gray-400'
                                    }`}
                                numberOfLines={1}
                            >
                                {label}
