    ...
    return (
        <SafeAreaView className="flex-1 bg-[#FAFAFA]" edges={['top']}>
            ...
            <Animated.View
                ...
                className="bg-[#1F2937] p-4 rounded-3xl shadow-2xl flex-row items-center border border-gray-700"
            >
            ...
            <View className="px-6 mb-6">
                <View className="flex-row bg-[#F3F4F6] p-1 rounded-2xl">
                    <TouchableOpacity
                        ...
                        className={`flex-1 py-3 rounded-xl items-center ${activeTab === 'upcoming' ? 'bg-white shadow-sm shadow-gray-200' : 'bg-transparent'}`}
                    >
                        <Text className={`font-bold ${activeTab === 'upcoming' ? 'text-[#1F2937]' : 'text-gray-500'}`}>
                            Upcoming
                        </Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                        ...
                        className={`flex-1 py-3 rounded-xl items-center ${activeTab === 'past' ? 'bg-white shadow-sm shadow-gray-200' : 'bg-transparent'}`}
                    >
                        <Text className={`font-bold ${activeTab === 'past' ? 'text-[#1F2937]' : 'text-gray-500'}`}>
                            Past
                        </Text>
                    </TouchableOpacity>
                </View>
                        ...
                        {UPCOMING_SESSIONS.length > 0 ? (
                            UPCOMING_SESSIONS.map((session, index) => (
                                <Animated.View
                                    ...
                                    className="bg-white p-6 rounded-3xl shadow-sm shadow-gray-200/50 mb-6 border border-gray-50"
                                >
                                    <View className="flex-row items-center justify-between mb-5">
                                        <View className="bg-purple-50 px-4 py-2 rounded-xl flex-row items-center border border-purple-100/50">
                                            ...
                                            <Text className="text-[#6B46C1] font-bold ml-2">
                                    ...
                                    <View className="flex-row items-center mb-6">
                                        ...
                                        <View className="ml-4 flex-1">
                                            <Text className="text-xl font-bold text-[#1F2937] tracking-tight">
                                                {session.mentorName}
                                            </Text>
                            ...
                            <Animated.View
                                ...
                                className="py-16 items-center justify-center border border-dashed border-gray-200 rounded-3xl bg-white/50"
                            >
                ...
                {activeTab === 'past' && (
                    <View>
                        {(PAST_SESSIONS ?? []).length === 0 ? (
                            // Empty state — shows instead of crashing when data is undefined/empty
                            <Animated.View
                                entering={FadeInDown.duration(500)}
                                className="py-16 items-center justify-center border border-dashed border-gray-200 rounded-3xl bg-white/50"
                            >
                                ...
                                <Text className="text-[#1F2937] font-bold text-lg">No past sessions</Text>
                                <Text className="text-gray-400 text-sm mt-1">
                                    Your completed sessions will appear here
                                </Text>

                                {/* Matches website's "View All Appointments" button */}
                                <TouchableOpacity className="mt-6 bg-[#6B46C1] px-6 py-3 rounded-xl">
                                    <Text className="text-white font-bold text-sm">Find a Mentor</Text>
                                </TouchableOpacity>
                            </Animated.View>
                        ) : (
                            // ✅ ?? [] ensures this never crashes even if PAST_SESSIONS is undefined
                            (PAST_SESSIONS ?? []).map((session, index) => (
                                <Animated.View
                                    key={session.id}
                                    entering={FadeInDown.delay(index * 100).duration(400)}
                                    className="bg-white p-5 rounded-3xl shadow-sm shadow-gray-200/50 mb-4 border border-gray-50"
                                >
                                    <View className="flex-row items-center justify-between">
                                        <View className="flex-row items-center flex-1">
                                            <Image
                                                source={{ uri: session.avatar }}
                                                className="w-14 h-14 rounded-full bg-gray-100 opacity-80"
                                            />
                                            <View className="ml-4 flex-1 pr-2">
                                                ...
                                                {/* ✅ Added mentorTitle — was missing in your original */}
                                        ...
                                        {!session.reviewed ? (
                                            <TouchableOpacity className="bg-yellow-50 px-4 py-2.5 rounded-xl border border-yellow-100">
                                                <Text className="text-yellow-700 font-bold text-xs tracking-wide">
                                                    Leave Review
                                                </Text>
                                            </TouchableOpacity>
                                        ) : (
                                            <TouchableOpacity className="bg-gray-50 px-4 py-2.5 rounded-xl border border-gray-100">
                                                <Text className="text-[#1F2937] font-bold text-xs tracking-wide">
                                                    Book Again
                                                </Text>
                                            </TouchableOpacity>
                                        )}
